...
def test_append_and_load():
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        ...
        repo.append_batch("B-001", "2025-06-01", "Grade A", {
            "Metric A": {"reps": [1.05, 1.10, 1.02], "lower_spec": 0.6, "upper_spec": 1.5},
            "Metric B": {"reps": [1500, 1520, 1490], "lower_spec": 1000.0, "upper_spec": float("nan")},
        })
...
def test_get_for_parameter():
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        ...
        adh = repo.get_for_parameter("Metric A")
        assert len(adh) == 1
...
def test_get_formulas():
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        ...
        repo.append_batch("B-001", "2025-06-01", "Grade A", {
            "Metric A": {"reps": [1.0], "lower_spec": float("nan"), "upper_spec": float("nan")},
        })
...
def test_get_parameters():
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        ...
        repo.append_batch("B-001", "2025-06-01", "Grade A", {
            "Metric A": {"reps": [1.0], "lower_spec": float("nan"), "upper_spec": float("nan")},
            "Metric B": {"reps": [1500], "lower_spec": float("nan"), "upper_spec": float("nan")},
        })

        params = repo.get_parameters()
        assert sorted(params) == ["Metric A", "Metric B"]
...
def test_append_batch_dedup():
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        ...
        repo.append_batch("B-001", "2025-06-01", "Grade A", {
            "Metric A": {"reps": [9.99, 9.99, 9.99], "lower_spec": 0.6, "upper_spec": 1.5},
        })

        df = repo.load_all()
        match = df[(df["batch_id"] == "B-001") & (df["parameter"] == "Metric A")]
...
def test_auto_migration_from_csv():
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        ...
        try:
            ...
            df = repo.load_all()
            assert len(df) == 2
            assert set(df["parameter"].tolist()) == {"Metric A", "Metric B"}
        finally:
...
def test_get_batch():
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        ...
        assert set(batch_df["parameter"].tolist()) == {"Metric A", "Metric B"}
...
def test_update_batch():
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        ...
        # Update Metric A values
        ok = repo.update_batch("B-001", "Grade A", "Metric A", {
            "reps": [2.0, 2.1, 2.2], "lower_spec": 0.6, "upper_spec": 1.5,
        })
        assert ok is True

        df = repo.get_for_parameter("Metric A")
...
def test_update_batch_not_found():
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        db_path = Path(tmpdir) / "test.db"
        repo = SqliteRepository(db_path, auto_migrate=False)
        ok = repo.update_batch("NOPE", "Grade A", "Metric A", {
...
def test_update_batch_rejects_invalid():
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        ...
        with pytest.raises(ValueError, match="Validation failed"):
            repo.update_batch("B-001", "Grade A", "Metric A", {
...
def test_delete_batch():
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
        ...
        deleted = repo.delete_batch("B-001", "Grade A", "Metric B")
        assert deleted is True

        df = repo.load_all()
        assert len(df) == 1  # only Metric A left
