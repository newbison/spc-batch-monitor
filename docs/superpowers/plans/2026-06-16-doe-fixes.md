...
Add these test functions to `tests/test_doe_analysis.py`:
...
Replace the entire content of `doe/analysis.py`:
...
Add to `tests/test_doe_optimization.py`:
...
In `tests/test_doe_optimization.py`, change line 81 from:
```python
...
Add to `tests/test_doe_designs.py`:
...
In `tests/test_doe_designs.py`, change line 69 from:
```python
...
Add to `tests/test_doe_persistence.py`:
...
Add to the imports at the top of `ui/engineer/doe_view.py` (after line 15):
...
```bash
grep -n "json.loads.*session\[" ui/engineer/doe_view.py
```
...
In `ui/engineer/doe_view.py`, lines 97-102, change:
```python
...
```bash
grep -n "json" ui/engineer/doe_view.py
```
...
```bash
python -m pytest tests/test_control_limits.py tests/test_rules.py tests/test_capability.py tests/test_validation.py tests/test_sqlite_repository.py tests/test_integration.py -v
```
